...
Page = ghostBookshelf.Model.extend({
    ...
    onSaving: function onSaving(model, attr, options) {
        ...
        if (prevTitle !== undefined && newTitle !== prevTitle && newStatus === 'draft' && !publishedAt) {
            ops.push(function updateSlug() {
                ...
                return ghostBookshelf.Model.generateSlug(Page, self.get('title'),
                    ...
                    .then(function then(slug) {
                        ...
                        return ghostBookshelf.Model.generateSlug(Page, prevTitle,
                            {status: 'all', transacting: options.transacting, importing: options.importing}
            ...
            ops.push(function updateSlug() {
                ...
                if (self.hasChanged('slug') || !self.get('slug')) {
                    // Pass the new slug through the generator to strip illegal characters, detect duplicates
                    return ghostBookshelf.Model.generateSlug(Page, self.get('slug') || self.get('title'),
                        {status: 'all', transacting: options.transacting, importing: options.importing})
                        .then(function then(slug) {
                            self.set({slug: slug});
                        });
    ...
    formatsToJSON: function formatsToJSON(attrs, options) {
        ...
        _.each(Page.allowedFormats, function (format) {
    ...
    toJSON: function toJSON(unfilteredOptions) {
        var options = Page.filterOptions(unfilteredOptions, 'toJSON'),
    ...
    orderDefaultRaw: function () {
        return '' +
            'CASE WHEN pages.status = \'scheduled\' THEN 1 ' +
            'WHEN pages.status = \'draft\' THEN 2 ' +
            'ELSE 3 END ASC,' +
            'pages.published_at DESC,' +
...
Pages = ghostBookshelf.Collection.extend({
    model: Page
});

// Extension for handling the logic for author + multiple authors
Page = relations.authors.extendModel(Page, Pages, ghostBookshelf);

module.exports = {
    Page: ghostBookshelf.model('Page', Page),
    Pages: ghostBookshelf.collection('Pages', Pages)
};
