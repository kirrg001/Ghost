...
class Urls {
    ...
    add(options, resource) {
        ...
        debug('cache', url, urlGenerator.toString());
        resource.take();
    ...
    removeUrl(url) {
        debug('removed', url, this.urls[url].urlGenerator.toString());
        common.events.emit('url.removed', this.urls[url].urlGenerator.toString());
    ...
    removeByResource(uid, resource) {
        ...
        urls.every((url) => {
            if (this.urls[url].resource === resource) {
                debug('remove', url, this.urls[url].urlGenerator.toString());
                delete this.urls[url];
                return false;
            }
