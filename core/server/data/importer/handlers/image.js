...
ImageHandler = {
    ...
    loadFile: function (files, baseDir) {
        var store = storage.getStorage(),
            ...
            imageFolderRegexes = _.map(globalUtils.url.STATIC_IMAGE_URL_PREFIX.split('/'), function (dir) {
        ...
        return Promise.map(files, function (image) {
            return store.getUniqueFileName(image, image.targetDir).then(function (targetFilename) {
                image.newPath = globalUtils.url.urlJoin('/', globalUtils.url.getSubdir(), globalUtils.url.STATIC_IMAGE_URL_PREFIX,
