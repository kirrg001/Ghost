...
class PostsImporter extends BaseImporter {
    ...
    addNestedRelations() {
        ...
        const run = (relations, target, fk) => {
            _.each(relations, (relation) => {
                ...
                if (!_.find(postToImport[target], {[fk]: relation[fk]})) {
     ...
     */
    replaceIdentifiers() {
        const run = (postToImport, postIndex, targetProperty, tableName, fk) => {
            ...
            _.each(postToImport[targetProperty], (object, index) => {
                let objectInFile = _.find(this.requiredFromFile[tableName], {id: object[fk]});

                if (!objectInFile) {
                    let existingObject = _.find(this.requiredExistingData[tableName], {id: object[fk]});
                    ...
                    if (!existingObject) {
                        ...
                        this.dataToImport[postIndex][targetProperty][index][fk] = existingObject.id;
                        return;
                    }
                }
                ...
                if (importedObject) {
                    this.dataToImport[postIndex][targetProperty][index][fk] = importedObject.id;
                    return;
                }
        ...
        _.each(this.dataToImport, (postToImport, postIndex) => {
            run(postToImport, postIndex, 'tags', 'tags', 'tag_id');
            run(postToImport, postIndex, 'authors', 'users', 'author_id');
        });
