...
describe('Schedules API', function () {
    ...
    scope.beforeDescribe = function (done) {
        ...
        async.waterfall([
            testUtils.teardown,
            testUtils.setup('clients', 'users:roles', 'perms:schedule', 'perms:init'),
            function(asyncDone) {
                testUtils.fixtures.addPermissionForClient({
                    'client_id': 2,
                    'permission_id': 1
                }).then(function() {
                    asyncDone();
    ...
    describe('fn: publishPost', function () {
        ...
        describe('error', function () {
            ...
            it('invalid params', function (done) {
                SchedulesAPI.publishPost({}, {id: 'bla', context: {user: testUtils.users.ids.author}})
                    .catch(function (err) {
                        should.exist(err);
                        (err instanceof errors.ValidationError).should.eql(true);
                        done();
